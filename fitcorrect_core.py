# ===========================
# fitcorrect_core.py
# FULL (Updated for minimal flow)
#
# ✅ One OpenCV window name for calibration + exercise
# ✅ Fullscreen by default
# ✅ Letterboxed rendering (no stretching)
# ✅ Single camera capture reused (no reopen)
# ✅ Buzz safety: off when not visible
# ===========================
...
try:
    cv2.setNumThreads(2)  # tweak: 2 or 4, depends on CPU
except Exception:
    pass
...
def _get_window_wh(window_name=WINDOW_NAME, fallback=(800, 450)):
    try:
...
def letterbox_to(img, target_w, target_h):
    """Resize with preserved aspect ratio + black bars."""
    h, w = img.shape[:2]
    if h <= 0 or w <= 0:
        return img
...
def calibration_phase(cap, user=DEFAULT_USER, difficulty="Standard"):
    ...
    with mp_pose.Pose(
        ...
        min_detection_confidence=0.6,   # slightly higher for cleaner calibration
...
def update_situp_rep(st, knee_val, hit_knee):
    # Basic sit-up rep latch using knee angle (minimal change).
    # Thresholds are tighter than squat because sit-up knee range is smaller.
...
def exercise_loop(cap, user=DEFAULT_USER, difficulty="Standard", profile=None, assigned_reps=12):
    if profile is None:
    ...
    reps = {
        ...
        "Sit-Up": rep_state_new(),   # ✅ add sit-up counting
    }
    ...
    with mp_pose.Pose(
        ...
        frame_i = 0
        POSE_EVERY_N = 2   # try 2 first, then 3 if needed
        last_res = None
        ...
        while True:
            ...
            cv2.putText(img, f"Exercise: {title}", (30, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 0), 2)
            cv2.putText(img, "Keys: Q=Quit  F=Toggle Fullscreen  N=Skip", (30, 80),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.75, (230, 230, 230), 2)

            # Target display
            if title == "Tree Pose":
                cv2.putText(img, f"Target: hold {tree_hold}s", (30, 110),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.85, (230, 230, 230), 2)
            else:
                cv2.putText(img, f"Target: {target_reps} reps", (30, 110),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.85, (230, 230, 230), 2)

            # ----------------------------
            # REST MODE (no evaluation)
            # ----------------------------
            if in_rest:
                send_off_all()
                remaining = int(max(0, rest_end_t - time.time()))

                cv2.putText(img, "REST", (30, 160),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.6, (255, 255, 0), 3)
                cv2.putText(img, f"Next in {remaining}s", (30, 210),
            ...
            if res is not None and res.pose_landmarks:
                ...
                if ok_now is None:
                    ...
                    if title in reps:
                        ...
                        if title == "Bodyweight Squat":
                            ...
                            update_situp_rep(st, knee, hit_knee)

                        # display_good based on rep phase
                        if st["in_rep"]:
                        ...
                        # show counts
                        cv2.putText(img, f"Reps OK: {st['good']}/{target_reps}", (30, 230),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                        cv2.putText(img, f"Reps BAD: {st['bad']}", (30, 270),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)

                        # ✅ completion -> rest -> next
                        if st["good"] >= target_reps:
                            send_off_all()
                            in_rest = True
                            rest_end_t = time.time() + REST_SECONDS
                            last_display[title] = None  # reset buzz latch
                            tree_ok_start = None
                            tree_done = False
                    ...
                    # ----------------------------
                        ...
                        if display_good is True:
                            ...
                            cv2.putText(img, f"Hold: {held:.1f}/{tree_hold}s", (30, 230),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                ...
                # UI label
                if display_good is True:
                    cv2.putText(img, "GOOD", (30, 160),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
                elif display_good is False:
                    cv2.putText(img, "ADJUST", (30, 160),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
                else:
                    cv2.putText(img, "NOT VISIBLE", (30, 160),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 165, 255), 2)

                if info:
                    cv2.putText(img, info, (30, 200),
                ...
                cv2.putText(img, "No body detected", (30, 160),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 165, 255), 2)
                send_off_all()
                last_display[title] = None
                tree_ok_start = None
...
if __name__ == "__main__":
    ...
    if profile_json_path:
        try:
            ...
            assigned_reps = int(ui_profile.get("assigned_reps", assigned_reps))
    ...
    try:
        if do_calibrate:
            ...
            if prof is None:
                ...
                raise SystemExit(1)
            exercise_loop(cap=cap, user=user, difficulty=difficulty, profile=prof)
        else:
            exercise_loop(cap=cap, user=user, difficulty=difficulty)
    finally:
        send_off_all()
        cap.release()
